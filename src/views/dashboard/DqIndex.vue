    ...
    <a-row :gutter="24">
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card :loading="isLoading" title="帐户余额" :total=" totalData.accountBalance?'￥' + totalData.accountBalance:'￥'+0">
          ...
          <div>
            <span style="color: #F95A24">待结算：￥ {{totalData.settleMoney?totalData.settleMoney:0}}</span>
          </div>
          <template slot="footer">
            <span>今日收入：{{totalData.moneyToday?'￥' + totalData.moneyToday:'￥'+0}}</span>
            <span style="float: right">本周收入：{{totalData.moneyWeek?'￥' + totalData.moneyWeek:'￥'+0}}</span>
          </template>
        </chart-card>
      </a-col>
...
<script>
  ...
  export default {
    ...
    created:function() {
      if(this.userInfo().channelId==2){
        let tem = [];
        for(let i in this.columns){
          if(this.columns[i].dataIndex != 'lowerShareMoney') tem.push(this.columns[i])
        }
        this.columns = tem
      }
     this.getIndexTotal();
    ...
    watch:{
      $route(val){
        if(val && val.name == 'isystem-QdIndex'){
          this.loadData();
          this.getIndexTotal();
        }
      }
