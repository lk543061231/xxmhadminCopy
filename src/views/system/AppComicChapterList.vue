<template>
  ...
  <a-card :bordered="false">
    ...
    <div class="table-operator" >
      <a-button @click="handleAdd" type="primary" icon="plus" v-if="viewModel==1">新增</a-button>
    ...
    <div >
      ...
      <a-table
        ...
        :loading="loading"
        :rowSelection="viewModel == 1?{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}:{}"
        @change="handleTableChange">
        
        <template slot="imgSlot" slot-scope="text">
          <span v-if="!text" style="font-size: 12px;font-style: italic;">无此图片</span>
          <img v-else :src="getImgView(text)" height="50px" alt="图片不存在" style="width:100px;font-size: 12px;font-style: italic;"/>
        </template>
    ...
    <appComicChapter-modal ref="modalForm" @ok="modalFormOk"></appComicChapter-modal>
  </a-card>
  </a-modal>
</template>
...
<script>
  ...
  import AppComicChapterModal from './modules/AppComicChapterModal'
  import { axios } from '@/utils/request'
  import { getAction } from '@/api/manage'

  export default {
    name: "AppComicChapterList",
    mixins:[JeecgListMixin],
    components: {
      AppComicChapterModal
    },
    data () {
      return {
        description: '漫画章节管理页面',
        visible:false,
        title:"章节列表",
        ...
        url: {
          list: "/app/appComicChapter/list",
          delete: "/app/appComicChapter/delete",
          deleteBatch: "/app/appComicChapter/deleteBatch",
          exportXlsUrl: "/app/appComicChapter/exportXls",
          importExcelUrl: "app/appComicChapter/importExcel",
    ...
    methods: {
      ...
      loadData(arg) {
        ...
        if(this.novelId) params.comicId = this.novelId;
      ...
      changeNeedGolds(status){
        if (this.selectedRowKeys.length <= 0) {
          ...
          that.$confirm({
            ...
            onOk: function () {
              that.loading = true;
              axios({
                url: "/app/appComicChapter/changeNeedGolds",
                method:'post' ,
       ...
       changeState(ids,status){
                             let that = this;
                             axios({
                               url: "/app/appComicChapter/changeState",
                               method:'post' ,
                               data: {ids: ids, status: status}
                             }).then((res) =>{
                               if (res.success) {
